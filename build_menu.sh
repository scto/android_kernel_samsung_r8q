...
#KERNEL_MAKE_ENV="LOCALVERSION=-afaneh92-ATHL"
...
variant(){
  ...
  findconfig=($(ls arch/arm64/configs/vendor/lineage_* 2>/dev/null))
  declare -i i=1
  shift 2
...
rebuilddefconfig(){
  variant
  echo "${GREEN}***** Copy config in Progress *****${STD}";
  mv arch/arm64/configs/vendor/lineage_${VARIANT}_defconfig arch/arm64/configs/lineage_${VARIANT}_defconfig
  echo "${GREEN}***** Rebuild defconfig in Progress *****${STD}";
  make $KERNEL_MAKE_ENV ARCH=arm64 CROSS_COMPILE=$BUILD_CROSS_COMPILE REAL_CC=$KERNEL_LLVM_BIN CLANG_TRIPLE=$CLANG_TRIPLE CFP_CC=$KERNEL_LLVM_BIN lineage_${VARIANT}_defconfig
  mv arch/arm64/configs/lineage_${VARIANT}_defconfig arch/arm64/configs/vendor/lineage_${VARIANT}_defconfig
  echo "${GREEN}***** Rebuild Done *****${STD}";
  pause 'continue'
 }
...
build(){
  ...
  [ ! -d "out" ] && mkdir out
  #make -j$(nproc) -C $(pwd) O=$(pwd)/out $KERNEL_MAKE_ENV ARCH=arm64 CROSS_COMPILE=$BUILD_CROSS_COMPILE REAL_CC=$KERNEL_LLVM_BIN CLANG_TRIPLE=$CLANG_TRIPLE CFP_CC=$KERNEL_LLVM_BIN vendor/afaneh_${VARIANT}_defconfig
  make -j$(nproc) -C $(pwd) O=$(pwd)/out $KERNEL_MAKE_ENV ARCH=arm64 CROSS_COMPILE=$BUILD_CROSS_COMPILE REAL_CC=$KERNEL_LLVM_BIN CLANG_TRIPLE=$CLANG_TRIPLE CFP_CC=$KERNEL_LLVM_BIN vendor/lineage_${VARIANT}_defconfig | tee $(pwd)/out/kernel_build.txt
  make -j$(nproc) -C $(pwd) O=$(pwd)/out $KERNEL_MAKE_ENV ARCH=arm64 CROSS_COMPILE=$BUILD_CROSS_COMPILE REAL_CC=$KERNEL_LLVM_BIN CLANG_TRIPLE=$CLANG_TRIPLE CFP_CC=$KERNEL_LLVM_BIN | tee $(pwd)/out/kernel_build.txt
...
show_menus(){
  ...
  echo "3. ${Under_Line}R${STD}ebuild defconfig"
